...
let typesList = [
    Helper.sOrderTypes.between,
    Helper.sOrderTypes.in,
    Helper.sOrderTypes.out,
]
...
async function getList(startDate, finishDate) {
    ...
    list.populate('AccountOut').populate('AccountIn')
    return list
}
...
async function getAccountOrders(id, startDate, finishDate) {
    let sords = ServiceOrder.find({
        $or: [{ AccountIn: id }, { AccountOut: id }],
        DateOrder: { $gte: startDate, $lt: finishDate },
    })
    sords.populate('AccountOut').populate('AccountIn')
    return sords
}
populateLists()
