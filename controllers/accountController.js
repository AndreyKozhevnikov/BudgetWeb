...
async function create_post(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            let found_acc = await Account.findOne({ Name: req.body.Name_frm })
            if (found_acc) {
...
async function tuneAccountResultObject(accRes, dateObject, isCreateFirstMonth) {
    ...
    accRes.accList.forEach((item) => {
        ...
        if (item.group != null) {
            let foundGroup = groupObject.groups.find((group) =>
                group.id.equals(item.group),
            )
            let itemResult = 0
...
async function processStatisticObjectAndMonthDates(
    ...
    const normAllPerDay = parseInt(process.env.NORM_ALL_PER_DAY) || 5000
