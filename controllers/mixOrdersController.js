...
async function listByAcc(req, res, next) {
    ...
    try {
        let orders = await orderController.getAccountOrders(
            accId,
            startDate,
            finishDate,
        )
        let sOrders = await serviceOrderController.getAccountOrders(
            accId,
            startDate,
            finishDate,
        )
        let fRecords = await fixRecordController.getAccountRecords(
            accId,
            startDate,
            finishDate,
        )
...
async function listByDate(req, res, next) {
    ...
    try {
        ...
        createAndShowMixOrdersList(
            ...
            [],
            res,
            Helper.getUrlDateString(startDate),
            null,
        )
    } catch (err) {
        console.log(err)
