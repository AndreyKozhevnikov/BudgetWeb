...
async function populatePaymentAccount(req, res, next) {
    ...
    var lst = await Order.find({ DateOrder: { $gte: dateToStart } }).populate(
        'PaymentType',
    )
...
function order_create_get_withNewLists(req, res, next) {
    populateAdditionalLists(order_create_get, {
        req: req,
        res: res,
        next: next,
    })
...
function createOrderFromRequest(req, isUpdate) {
    let order = new Order({
        ...
        IsShouldNotCalculate: Boolean(req.body.fIsShouldNotCalculate),
