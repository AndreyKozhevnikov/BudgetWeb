...
async function getTheLastFixRecordsDate() {
    ...
    if (lastFRecord == null) return new Date()
...
async function getList(startDate, finishDate) {
    let list = Helper.getListByDates(
        FixRecord,
        startDate,
        finishDate,
        'DateTime',
...
async function createTotalIncoming(req, res, next) {
    ...
    for (let inOrder of sInOrders) {
        ...
        await createFixRecord(FRecordTypes.TotalIncoming, dt, null, inOrder.sum)
    }
    for (let order of orders) {
        ...
        await createFixRecord(FRecordTypes.TotalExpense, dt, null, order.sum)
    }
...
async function showTotalSumsChart(req, res, next) {
    ...
    res.render('totalSumsChart.pug', {
        chartListRub: chartListRub,
        chartListDram: chartListDram,
    })
}
