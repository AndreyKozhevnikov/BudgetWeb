...
async function fullRestore(req, res, next) {
    ...
    for (let entityCollectionProperty in backupObject) {
        ...
        for (let i = 0; i < entityCollection.length; i++) {
            ...
            // }
            if (
                savedEntity.DateOrder != null &&
                new Date(savedEntity.DateOrder) < new Date('01-jan-25')
            ) {
                continue
            }
            if (
                savedEntity.DateTime != null &&
                new Date(savedEntity.DateTime) < new Date('01-jan-25')
            ) {
                continue
            }
...
async function index(req, res) {
    ...
    let results = await Promise.all([tagCount(), orderCount()])
    let today = new Date()
